...
1. 什么是信号量？它与软件同步方法的区别在什么地方？
 ...
 + 区别：
   + 软件同步是平等线程间的一种同步协商机制，不需要进入操作系统内核；
   + 信号量是由地位高于进程的管理者OS协调的同步机制；

2. 自旋锁为什么无法按先来先服务方式使用资源？
  + 因为自旋锁是每个轮循到的进程进行判断，看能否进入临界区，第一个检测到可以进入临界区的进程很大可能不是第一个申请进入临界区的进程
...
```
 ...
 + 参考回答： 它的问题是，不能按FIFO进行信号量申请，这是一种Hansen的信号量机制。
...
3. 为什么在生产者-消费者问题中先申请互斥信号量会导致死锁？
  + 假设生产者先申请信号量进入临界区，但缓冲区已满，生产者进入等待状态，但消费者由于无法进入临界区而无法消耗资源，从而出现死锁
  + 同样如果消费者进入临界区，但还没有任何资源，消费者进入等待状态，但生产者由于无法进入临界区而无法生产资源，从而出现死锁
 > 在两种情况下会出现循环等待：
...
4. 为什么互斥信号量的实现比资源信号量的实现要简单？请说明．
  + 互斥信号量只要设置value的最大值为1，但资源信号量的value的最大值为资源总数。
...
1. 在读者-写者问题的读者优先和写者优先在行为上有什么不同？
  + 读者优先：在读者等待队列中存在等待读者时，写者必须持续等待。
  + 写者优先：在写着队列中存在即将写的进程时，后来的读者需要等到写者写完再读。
...
```
  + 有问题，再wait中应该先进行x.count++，再进行锁的释放，同样在signal里面，也应该先增加next.count++再进行V(x.sem)。不然会有这样的情况，在执行V(x.sem)后，进行了线程的切换，切换到原进程，该进程继续向后执行，进入退出去，由于还没有设置next_count++，所以会直接通过V(mutex)结束运行，之后再切换到signal线程，会一直等待在P(next.sem)中。
