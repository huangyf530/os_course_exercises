...
1. 尝试描述下列处理机调度算法的工作原理和算法优缺点。
   ...
   - 短进程优先算法（SPN、SJF）
     ...
     - 优点：
       - 周转时间最短
   - 最高响应比优先算法（HRRN）
     - 工作原理：按响应比R=(w+s)/s进行排序
     - 缺点：
       - 需要每次对进程排序，开销大
     - 优点：
       - 既考虑进程的执行时间也兼顾了长进程不会出现饥饿
...
2. 为什么短进程优先算法的平均周转时间最优？
  + 任何其他的排序都可以经过短进程优先算法的排序经过相邻进程的左右互换实现，可以证明每次交换平均周转时间都会增加

 > 可以证明知进程优先算法的调度顺序的平均周转时间是最短的。

3. 如何调试你的调度算法？
  + 监控算法的执行状态，根据需要观察相应的指标，分析算法的特征并进行优化。
...
1. 尝试描述下列处理机调度算法的工作原理和算法优缺点。
   - 时间片轮转算法（RR）
     ...
     - 缺点：
       - 引入很多进程切换的开销
   - 多级反馈队列算法（MLFQ）
     - 工作原理：多个不同调换策略的就绪进程队列，进程可在不同的队列中移动，优先级越低的队列时间片越长。
     - 优点：
       - 可以提高响应时间，I/O密集型的进程会停留在高优先级
       - 考虑了不同进程的不同需求
     - 缺点：
       - 实现复杂
       - 可能出现低优先级进程队列饥饿
   - 公平共享调度算法（FSS）
     - 工作原理：把资源按照用户进程的比例分配，没有达到资源利用率的用户组可以得到更高的优先级
     - 优点：
       - 资源的分配更加公平
     - 缺点：
       - 实现比较复杂
...
3. 请描述在MLFQ中，如何提升或降低进程的优先级？
  + 如果一个进程没有占用完一个时间片，则降低该进程的优先级，进入低优先级的队列，获得更长的时间片。
  + 如果一个进程在一个时间片内停止，就提高该进程的优先级
