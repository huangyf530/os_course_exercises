...
2. 不同特权级有什么区别？
  * 不同特权级对于不同的数据段、代码段有不同的访问权限。
  * 不同的特权级还有着不同的指令，有些特权指令需要在一定的特权等级才可以使用。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
  * 中断响应中：需要CPL <= DPL（门）且 CPL >= DPL（段）
  * 函数调用和指令执行时：需要MAX（CPL，RPL）<= DPL（段）
...
1. 一条指令在执行时会有哪些可能的特权级判断？
  * 在访问段时进行特权级判断，需要MAX（CPL，RPL）<= DPL（段）
  * 在访问门时进行特权级判断，需要CPL <= DPL（门）且 CPL >= DPL（段）

2. 在什么情况下会出现特权级切换？
  * 在进行中断处理时需要进入操作系统特权级

3. int指令在ring0和ring3的执行行为有什么不同？
  * 在ring0中执行时，只需要保存CS，EIP的信息，Error Code等内容
  * 在ring3中执行时，需要进行特权级的转换，也要将用户态SS，ESP的信息压入栈中

4. 如何利用int和iret指令完成不同特权级的切换？
  * 用户态->内核态：通过int指令，进入中断服务例程返回前，修改栈中的数据，主要是将CS和中的CPL设为0，并删除SS和ESP的信息，执行iret指令后实现特权级的变化。
  * 内核态->用户态：通过中断的方法，修改栈中的数据，在栈中加入SS和ESP，并设置SS的RPL为3，修改CS的CPL为3之后调用iret后直接将上述信息放在对应的寄存器中，实现特权级的切换。


5. TSS和Task Register的作用是什么？
  * TSS保存不同特权级的堆栈等信息，用于特权级切换时找到对应的堆栈地址。
  * Task Register保存TSS的段选择子
...
1. 一条指令执行时最多会出现多少次地址转换？
  * 1 + n次：1次段式地址转换，n级页表需要n次页式地址转换。
2. 描述X86-32的MMU地址转换过程；
  * 首先通过段式地址转换得到完整的虚拟地址。
  * 虚拟地址经过二级页表的转换得到最后的物理地址。
